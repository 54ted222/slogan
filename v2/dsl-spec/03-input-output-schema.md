...
`input_schema` 定義 workflow instance 建立時 MUST 提供的資料結構。使用 JSON Schema 子集。
...
`output_schema` 定義 workflow 完成時 `return` step 回傳的資料結構。語法與 input schema 相同。
...
- 若 workflow 沒有定義 `output_schema`，`return` step 的 output 不做驗證
...
- 沒有 `output_schema` → workflow 完成時不驗證 return output
