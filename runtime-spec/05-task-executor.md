...
引擎 MUST NOT 自動跟隨 HTTP 3xx redirect。3xx 視為非 2xx，依一般結果處理規則判定失敗。

### Response 大小限制

引擎 MUST 限制 response body 讀取大小（預設 10 MB）。超過上限時：

1. 中止讀取
2. 關閉連線
3. 回報失敗（error code = `task_failed`，message 包含大小限制資訊）

### Chunked Transfer Encoding

引擎 MUST 支援 chunked transfer encoding（HTTP/1.1 標準）。引擎持續讀取 chunks 直到完成或超過大小限制。Response body 的完整 JSON 在所有 chunks 接收完畢後解析。

### Artifact 限制

HTTP backend **不支援** artifact resource binding。若 task step 同時使用 `backend.type: http` 且定義了 `resources`，引擎 MUST 在驗證階段（DRAFT → VALIDATED）產生錯誤。

原因：HTTP backend 以 JSON body 傳輸資料，無法像 stdio backend 將 artifact 綁定至本地檔案系統路徑。若需透過 HTTP 傳遞檔案，task handler 應自行透過 `input` 取得 artifact URI 並處理下載 / 上傳。
...
| `artifacts` | map \| null | 綁定的 artifact（僅 step 有 `resources` 時存在，見下方說明） |

#### artifacts 欄位

若 step 透過 `resources` 綁定了 artifact，引擎 MUST 在 request 中加入 `artifacts` map。Key 為 resource 的 `name`，value 包含：

| 欄位 | 型別 | 說明 |
|------|------|------|
| `path` | string | 本地檔案路徑（引擎準備的暫存路徑） |
| `content_type` | string \| null | MIME type（`write` 新建時為 null） |
| `size` | integer \| null | 檔案大小 bytes（`write` 新建時為 null） |
| `access` | string | `read`、`write`、`read_write` |

- Tool 透過 `path` 直接以檔案系統讀寫 artifact 內容
- `read` access 的檔案為唯讀，tool MUST NOT 修改
- `write` / `read_write` 的檔案，tool 寫入後引擎會自動上傳至 storage backend
- Tool 不需知道 storage backend 的實作（S3、local filesystem 等）
...
1. 組合環境變數：
   - 系統 env + task definition 的 `backend.env`（若有）
   - 求值 `backend.env` 中的 CEL 表達式
   - 若有 idempotency key → `SLOGAN_IDEMPOTENCY_KEY` 環境變數
   - `SLOGAN_INSTANCE_ID` = workflow instance ID
   - `SLOGAN_STEP_ID` = step ID
2. 啟動 shell process（使用 `backend.shell`，預設 `/bin/sh`）執行 `backend.command`
...
- stderr 內容寫入 execution_log（作為 tool 的 log 輸出）
- 不影響成功 / 失敗判定
- 記錄上限：建議 64KB，超過部分截斷

### 大 Payload 處理

引擎 MUST 限制 stdin / stdout 的 JSON 大小（預設 10 MB）。

- Stdin（request）：引擎在序列化後檢查大小。超過上限 → 不啟動 process，step FAILED
- Stdout（response）：引擎持續讀取直到 EOF 或超過上限。超過上限 → 終止 process，step FAILED

### Signal 與 Process Group

引擎 MUST 以 process group（或 session）方式管理 stdio tool：

1. 啟動 tool 時建立新的 process group
2. Timeout 或取消時，向整個 process group 發送 SIGTERM
3. `timeout_kill_after` 後向整個 process group 發送 SIGKILL

這確保 tool 啟動的子 process（如 shell pipeline 中的多個 command）都被正確終止。

### Resource Limit

引擎 SHOULD 使用 OS 機制限制 stdio tool 的資源：

| 機制 | 說明 |
|------|------|
| `ulimit` / `setrlimit` | 限制記憶體、file descriptors |
| Process group | 確保子 process 共享限制 |
| 引擎設定 | 可按 task action 設定不同限制 |
