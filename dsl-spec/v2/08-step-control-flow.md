...
```yaml
- type: if
  expr: CEL expression          # MUST, 回傳 boolean
  then: [...]                   # MUST, step 陣列
  else: [...]                   # MAY, step 陣列
  on_error: [...]               # MAY
```
...
- `expr` 求值為 `true` → 執行 `then` 陣列
- `expr` 求值為 `false` → 執行 `else` 陣列（若有），否則 SKIP
...
- `expr` 求值一次，結果與每個 `case.value` 做相等比較
