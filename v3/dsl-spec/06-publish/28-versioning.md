...
詳見 [10-step-terminal](../02-steps/10-step-terminal.md)。
...
`if` 和 `switch` step 的 `expr` 欄位已改名為 `when`：
...
# v3
- type: if
  when: ${ steps.order.output.status == "cancelled" }
  then: [...]
```
...
# v3
- type: switch
  when: ${ steps.order.output.priority }
  cases: [...]
```
...
詳見 [15-agent-skills](../03-agent/15-agent-skills.md) 與 [18-toolset-definition](../04-resources/18-toolset-definition.md)。
...
|------|-------------|------|----------|
| Agent 一等公民 | `kind: Agent`、`type: agent` step | AI agent 角色模板，由 workflow step 組裝能力 | [13-agent-definition](../03-agent/13-agent-definition.md) |
| Saga 補償 | `type: saga` step、step-level `compensate` | 交易性補償模式 | [12-step-saga](../02-steps/12-step-saga.md) |
| Continue-as-new | `return` + `renew: true` | Workflow 續接，可指定新版本 | [10-step-terminal](../02-steps/10-step-terminal.md) |
| Toolset | `kind: Toolset` | 統一 tools + skills 集合 | [18-toolset-definition](../04-resources/18-toolset-definition.md) |
| Resources | `kind: Resources` | MCP servers、string templates、model aliases | [19-resources-definition](../04-resources/19-resources-definition.md) |
| Project | `kind: Project` | Definition 檔案 project 組織 | [20-project](../04-resources/20-project.md) |
| Instance Labels | workflow instance | 鍵值對分類與查詢 | [24-instance-labels](../05-runtime/24-instance-labels.md) |
| 兩段式 Tool 命名 | task / agent tools | `namespace.action` 格式 | [06-step-task](../02-steps/06-step-task.md)、[14-agent-tools](../03-agent/14-agent-tools.md) |
...
1. 複製 v2 definition，修改 apiVersion 為 v3
     ↓
2. 全域搜尋替換 expr → when（if / switch steps）
     ↓
...
| 主題 | 文件 |
|------|------|
| Kind 定義與 apiVersion | [01-kind-definitions](../01-core/01-kind-definitions.md) |
| Task step 與兩段式命名 | [06-step-task](../02-steps/06-step-task.md) |
| 控制流程（when） | [08-step-control-flow](../02-steps/08-step-control-flow.md) |
| Return 與 continue-as-new | [10-step-terminal](../02-steps/10-step-terminal.md) |
...
| Agent Skills | [15-agent-skills](../03-agent/15-agent-skills.md) |
...
| Instance Labels | [24-instance-labels](../05-runtime/24-instance-labels.md) |
