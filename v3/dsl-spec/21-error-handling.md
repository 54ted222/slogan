...
| **Block-level** | 控制流程 step 上的 `on_error`（if / switch / foreach / parallel / saga） | 該 block 內未被 step-level 捕捉的錯誤 |
...
| `saga` | 捕捉 saga 補償後的最終錯誤（見 [12-step-saga](12-step-saga.md)） |
...
## on_error 與 saga compensate 的關係

`on_error` 與 saga `compensate` 是兩種互補的錯誤處理機制，執行順序明確：

1. Step 失敗 → 先執行該 step 的 `on_error`（若有）
2. `on_error` 正常完成 → 錯誤已處理，step 不再為 FAILED → **不觸發** saga 補償，workflow 繼續
3. `on_error` 不存在、重新拋出錯誤、或自身失敗 → step 仍為 FAILED → **觸發** saga 補償（若 step 位於 saga 範圍內）

| 機制 | 觸發範圍 | 用途 |
|------|----------|------|
| `on_error` | 單一 step 失敗 | 錯誤處理（降級、記錄、重新拋出） |
| `compensate` | Saga 範圍內 step 失敗 | 反向回復已完成的副作用 |

兩者互補而非互斥：`on_error` 處理「如何應對當下的錯誤」，`compensate` 處理「如何回滾已完成的操作」。典型模式是在 `on_error` 中進行告警或降級嘗試，若無法恢復則讓 saga 補償接手回滾。

> 詳見 [12-step-saga](12-step-saga.md) 的「與 on_error 的關係」章節。
...
重點：retry 會在 `on_error` 之前全部用盡。`on_error` 在 saga 補償之前執行。
...
> **重要**：`agent_tool_failed` 不會直接導致 agent step 失敗。Tool 失敗的結果會作為 tool result 送回 LLM，由 LLM 決定下一步（重試、使用其他 tool、或放棄）。只有當 LLM 明確表示無法完成任務，或達到迭代 / tool call 上限時，agent step 才會 FAILED。此錯誤碼記錄於 `agent_tool_calls` 表中，而非直接觸發 `on_error`。
>
> Agent 錯誤碼的完整定義詳見 [13-agent-definition](13-agent-definition.md)。
...
| 錯誤碼 | 觸發條件 | 說明 |
|--------|---------|------|
| `compensation_failed` | Saga 補償流程中有 compensate step 失敗 | 見 [12-step-saga](12-step-saga.md) |
...
on_error:
  - type: if
    condition: ${ error.code == "payment.insufficient_funds" }
    then:
      - type: emit
...
steps:
  ...
  - id: analyze
    ...
    on_error:
      - type: if
        condition: ${ error.code == "agent_llm_error" }
        then:
          - type: emit
