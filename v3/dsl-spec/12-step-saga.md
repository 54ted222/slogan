...
Saga 範圍內任一 step 狀態為 FAILED（且未被該 step 自身的 `on_error` 恢復），即觸發補償流程。
...
2. `on_error` 執行後，若 step 仍為 FAILED → 觸發 saga 補償
3. `on_error` 成功恢復（step 不再為 FAILED）→ 不觸發補償，saga 繼續執行
