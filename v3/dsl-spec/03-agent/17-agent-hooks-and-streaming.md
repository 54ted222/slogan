...
hooks:
  on_before_tool_call:
    - type: if
      condition: ${ hook.tool_name == "order.delete" }
      then:
        - type: task
          action: audit.log
          input:
...
hooks:
  on_before_tool_call:
    - type: if
      condition: ${ hook.tool_name in ["order.delete", "payment.refund", "customer.deactivate"] }
      then:
        - type: task
          action: audit.log
          input:
...
- [09-step-events](09-step-events.md) — emit 與 wait_event step
