...
| 欄位 | 型別 | 必填 | 說明 |
|------|------|------|------|
| `apiVersion` | string | MUST | 固定值 `secret/v3` |
...
`secret` namespace 可在以下位置使用：

| 位置 | 範例 |
|------|------|
| Workflow step `input` | `${ secret.PAYMENT_API_KEY }` |
| Task definition `backend` | `${ secret.PAYMENT_API_KEY }` |
| Agent definition `system_prompt` | `${ secret.INTERNAL_ENDPOINT }` |
| Resources definition MCP server `env` | `${ secret.MCP_API_KEY }` |
...
# Agent definition 的 system_prompt 中（詳見 13-agent-definition）
system_prompt: |
  你是付款助手。使用 endpoint ${ secret.PAYMENT_INTERNAL_URL } 查詢交易。

# Resources definition 的 MCP server env 中（詳見 19-resources-definition）
mcp_servers:
  - name: db_server
    command: "npx"
