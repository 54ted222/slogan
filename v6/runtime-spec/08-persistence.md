...
- publisher 沿用 outbox 表內 `committed_at IS NOT NULL AND published_at IS NULL` 的紀錄重投（冪等，訂閱端以 `event.id` dedup）；注意 `committed_at` 由 transaction commit 時 DB 填入，尚未 commit 的 INSERT 對其他 session 不可見（MVCC），故讀到的 row 必然已 committed
...
- `resource_pool` 的 rows **不在** 終結 transaction 內刪除；destroy hook 於 checkpoint commit 之後 best-effort 執行（見 `02-instance-lifecycle.md`），執行結束（成功或失敗）由 resource pool 背景 job DELETE 該 row。未成功 destroy 的 row 保留於清理重試佇列，依下列重試政策執行

### lifecycle.destroy 重試政策

| 參數 | 預設值 | 說明 |
|------|--------|------|
| `engine.destroy_retry_max_attempts` | 5 | 包含首次執行的總嘗試次數；耗盡後視為 abandon |
| `engine.destroy_retry_initial_delay` | 30s | 首次失敗後的第一段 sleep |
| `engine.destroy_retry_backoff` | `exponential`（factor 2） | 固定 `exponential`；每次 sleep = `min(initial_delay × 2^(attempt-1), max_delay)` |
| `engine.destroy_retry_max_delay` | 10m | backoff 封頂 |

重試流程：

1. destroy 失敗 → 發 `lifecycle.destroy_failed` observability 事件（含 `{instance_id, resource_key, error, attempt, next_retry_at}`）
2. 排入清理重試佇列（`destroy_retry_queue` 邏輯表或 resource_pool 表新增 `destroy_next_retry_at` 欄位）
3. 背景 job 於 `destroy_next_retry_at <= now()` 時重跑該 destroy hook；累計 `attempts += 1`
4. 達 `destroy_retry_max_attempts` 仍失敗 → 發 `lifecycle.destroy_abandoned`（含 `{instance_id, resource_key, error, attempts}`）→ DELETE resource_pool row（加 audit log 記錄放棄事實）；**不**保留 row 以免累積僵屍記錄（ops 工具需以 observability 事件排查資源洩漏）
5. 所有參數可由 engine config 覆寫；生產環境若需人工介入前先保留 row，MAY 將 `destroy_retry_max_attempts` 設極大值（不建議）
