...
def evaluate(expr_string, ctx) -> Value:
    ...
    if ast.depth > engine.max_cel_ast_depth:
        raise ExpressionError(type="too_deep", message=..., fragment=expr_string)
    try:
        result = run(ast, ctx)
        ...
        raise ExpressionError(type="division_by_zero", message=str(e), fragment=expr_string)
    except IndexError as e:  # list / string 越界
        raise ExpressionError(type="out_of_bounds", message=str(e), fragment=expr_string)
    if sizeof(result) > engine.max_cel_result_bytes:
        raise ExpressionError(type="too_large", message=..., fragment=expr_string)
    return result
```
...
- Engine MUST 在構造 `ErrorObject` 前對 `error.message` 與 `error.fragment` 做過濾：
  ...
  - **替換順序 MUST 為長度降序**：對 secret 明文集合按 length(value) desc 排序後逐一替換；理由：若 secret A = `"abc"`、secret B = `"abcdef"` 同時存在，先替換較長者 B 避免 A 的部分匹配殘留未遮蔽明文（範例：訊息 `"token=abcdef"` 若先替換 A → `"token=***def"`，殘留 `def` 為 B 的部分明文）
  - 長度相等者：以字典序替換（確定性，便於 test 驗證）
  - **重複明文去重**：Set 化後排序；同一明文於多個 secret key 下出現時只替換一次
...
| `expression_error.too_deep` | AST 深度超限（見下「求值不變式」的深度限制） |
| `expression_error.too_large` | 結果大小超限（見下「求值不變式」的大小限制） |
| `expression_error.template_eval` | template 字串組裝失敗 |
| `expression_error.mapping` | stdout.mapping / response.mapping 求值失敗 |
| `expression_error.division_by_zero` | 整數 / 浮點除法或取模除以 0 |
| `expression_error.out_of_bounds` | 字串 / list 索引 / 切片越界 |

呼叫端可選擇將子型別併為頂層 `expression_error`；CEL fragment 一律放入 `error.message`。本表與 [09-error-model.md 「表達式 / 求值」章](09-error-model.md#表達式--求值) 同步；跨檔新增錯誤碼時 MUST 同時更新兩處以避免漂移。
...
**錯誤定位**：當模板中某個 `${ ... }` 求值失敗，`ErrorObject` MUST 攜帶：

- `error.fragment`：失敗的該段 CEL 原文（單一 `${ ... }` 內容，不含定界符與其他 segment）
- `error.details.segment_index`：0-based 索引，表示失敗的是第幾個 `${ }`（left-to-right 計數，literal 不計）
- `error.details.template`：完整模板字串（含所有 `${ }`），方便使用者對照

例：`"order ${ steps.x.output.id } total ${ steps.y.output.amount }"` 中第 2 個 `${ }` 失敗 → `segment_index == 1`、`fragment == "steps.y.output.amount"`。
...
- `a && b`：若 `a == false`，`b` **不求值**
- `a || b`：若 `a == true`，`b` **不求值**
- `cond ? x : y`：僅求值 `cond` 及被選中的分支

**對 identifier_not_found 的保護**：短路可抑制右側 CEL 的異常。例：

```
has(steps.foo.output) && steps.foo.output.id      # steps.foo SKIPPED / 未終態 → 左側 false → 右側不求值
default(steps.foo.output.id, "fallback")          # output 為 null → default 回 fallback，不拋異常
has(vars.counter) ? vars.counter : 0              # vars.counter 未寫 → 左側 false → 右側返回 0
```

短路**不抑制**型別錯誤：`a && b` 的 `a` 非 boolean → `expression_error.type_error`（不短路）。
