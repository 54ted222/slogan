...
**根目錄 definition（`project.name == ""`）的 scope 語意**：

- `scope: workflow`：語意不變（僅當前 instance 內流通）
- `scope: project`：由於根目錄無 project，路由時等價於「僅匹配同屬根目錄 definition（`project.name == ""`）的 trigger / wait」；**不**自動升級為 `global`，亦不會匹配任何具名 project 內的訂閱者
- `scope: global`：語意不變（跨所有 project 與根目錄 definition）
- 根目錄 definition 的 event trigger / wait 亦套用同規則：`scope: project` 的 trigger 僅匹配根目錄 emit 出的 `scope: project` 事件
