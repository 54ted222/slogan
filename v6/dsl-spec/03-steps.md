...
- `failure_policy`：
  - `fail_fast`（預設）：任一迭代 FAILED → 取消尚未開始的迭代，已啟動的迭代繼續執行至終態；foreach FAILED，`error` **直接繼承**首個失敗迭代的 error（`error.type` 即該迭代的原錯誤類型，`error.step_id` 為首個失敗者的 step id）；不包裝為新 error.type
...
- `failure_policy`（語意與 foreach 對應）：
  - `fail_fast`（預設）：任一 branch FAILED → 取消其他 in-flight branches、等待至其終態；parallel FAILED，`error.type == "branch_failed"`、`error.details.branch_index` 為觸發分支、`error.cause` 為該分支原 error
