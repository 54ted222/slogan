...
### timeout 與 retry 的時序關係

step-level `timeout` 與 `retry` 共存時的計時邊界：

- `timeout` **計時起點** = step 首次進入 RUNNING 的時刻（寫入第一次 checkpoint A 之時）；**不**隨 attempt 重置
- `timeout` **涵蓋範圍** = 首次進入 RUNNING 起、至 step 達終態（SUCCEEDED / FAILED）為止的**整段 wall-clock**，**含**所有 retry 間的 `delay` sleep、所有 attempt 的實際執行時間
- 即 `timeout = 10s` + `retry.max_attempts = 3` + `retry.delay = 4s`：若第 1 次 attempt 花 3s FAILED → sleep 4s → 第 2 次 attempt 花 5s → **超時（3+4+5=12s > 10s）**，step FAILED 以 `timeout` 為終態（不繼續 attempt 3）
- Engine 於每次 attempt 開始前 check 剩餘 deadline；若剩餘時間不足以容納下次 delay，直接以 `timeout` 終結，避免「為了 sleep 而 sleep」
- 實作：deadline 於首次 RUNNING checkpoint 寫入（`timeout_resolved_ms` 類似語意但 per-step；寫入 `steps.deadline` 欄位，持久化後 engine restart 沿用）
- **缺省 `timeout`**：step 無獨立上限，僅受 workflow / function `config.timeout` 與外部 cancel 約束
...
- Output 為被選中分支**最後一個已終結（SUCCEEDED / FAILED / SKIPPED）** step 的 output；若選中分支為空陣列（`then: []` / `else: []`）或所有 step 皆因 `when: false` SKIPPED → if 的 output 為 `null`、status 為 SUCCEEDED
- `condition` 為 `false` 但未宣告 `else` → if SUCCEEDED，output 為 `null`（等價於 `else: []`）
...
- Output 為被選中 case（或 default）最後一個已終結 step 的 output；若選中 case 為空陣列或所有 step 皆因 `when: false` SKIPPED → output 為 `null`、status SUCCEEDED
- **未被選中的 case 不執行**：未匹配 case 的 `value` CEL 求值失敗不影響 switch 整體（僅當該 case 被檢查時才求值；若前面的 case 先匹配，後續 case 連 `value` 都不求值）
...
- `concurrency`：同時執行的迭代上限；`async` 不影響此值。
  - 字面整數：MUST 為正整數（≥ 1）；0 / 負整數 / 非整數 → 載入失敗，`error.type == "invalid_concurrency"`、`details.reason: "non_positive_literal"`
  - CEL 表達式：step 進入 RUNNING 前求值，結果 MUST 為正整數；結果為 0 / 負數 / 非整數 → step FAILED，`error.type == "invalid_concurrency"`、`details.reason: "non_positive_runtime"`、`details.evaluated` 為實際值
  - 缺省 → 1（序列執行）
  - 上限：受 `engine.max_concurrency_per_step`（預設 100）封頂；超過時以封頂值執行，不報錯（僅發 observability warning）
...
- `failure_policy`：
  - `fail_fast`（預設）：任一迭代 FAILED → 取消尚未開始的迭代，已啟動的迭代繼續執行至終態；foreach FAILED，`error` **直接繼承**首個失敗迭代的 error（`error.type` 即該迭代的原錯誤類型，`error.step_id` 為首個失敗者的 step id，`error.details.failed_index` 為首個失敗迭代的 0-based index）；不包裝為新 error.type。注：parallel fail_fast 以 `error.type == "branch_failed"` 包裝（見下 parallel 章節），兩者語意對稱但錯誤包裝策略不同（foreach 保留原 error.type 以便 catch 分流，parallel 以統一 type 標示）
  - `continue`：迭代失敗不中止迴圈；所有迭代完成後若有任一失敗 → foreach FAILED，`error.type == "foreach_failed"`、`error.details.failed_indices` 為失敗索引清單、`error.cause` 為首個失敗迭代的 error；output 包含成功迭代的結果（失敗位置為 `null`）。
  - `ignore`：迭代失敗視為 `null` output；foreach 永遠 SUCCEEDED。
- 迭代內部 step 的 `catch` 在 `failure_policy` 之前求值；被 catch 處理的失敗不視為迭代失敗。
- **空 items / count 為 0**：foreach 立即 SUCCEEDED，`output = []`；`failure_policy` 不適用（無迭代可失敗），`error` 欄位不存在（`steps.<foreach_id>.error` 為 unset），CEL 中 `has(steps.<foreach_id>.error)` 為 `false`。`do` block 不執行（含內部 step 的 `when` / `catch`）；checkpoint 僅寫入 foreach 本身的 SUCCEEDED 終態，不寫入 `failed_indices` 或任何迭代狀態，以免 replay 時與 `fail_fast` / `continue` 的未啟動分支混淆。
...
- `branches` MUST 為**非空陣列**；`branches: []` → 載入失敗，`registry.invalid_workflow_definition`、`details.reason: "empty_parallel_branches"`；不沿用 foreach 空 items 的「立即 SUCCEEDED」語意，因為 parallel 的 branches 為定義結構（不像 foreach items 來自運行期求值）
- 每個 `branches[i].steps` 亦 MUST 非空；空 branch 載入失敗，`details.reason: "empty_parallel_branch_steps"`
- `failure_policy`（語意與 foreach 對應）：
  - `fail_fast`（預設）：任一 branch FAILED → 取消其他 in-flight branches、等待至其終態；parallel FAILED，`error.type == "branch_failed"`、`error.details.branch_index` 為觸發分支（0-based）、`error.cause` 為該分支原 error
  - `continue`：branch 失敗不中止其他 branches；全部結束後若有任一失敗 → parallel FAILED，`error.type == "parallel_failed"`、`error.details.failed_indices` 為失敗 branch 索引、`error.cause` 為首個失敗 branch 的 error
  - `ignore`：branch 失敗視為 `null` output；parallel 永遠 SUCCEEDED

### Branch 作用域

各 branch 為**獨立作用域**，規則與 foreach 迭代類似：

- `steps.*` 僅可見**本 branch 已完成**的 step；branches 間無法互讀 output。跨 branch 共享資料請於 parallel 前以 `assign` 寫入 `vars`。
- **vars 寫入語意**（與 foreach 迭代一致，見 `runtime-spec/10-concurrency.md` 的「Foreach 並行 iteration 的 vars 寫入」）：`vars` 為 instance-level namespace，branch 內 `type: assign` 的寫入**共享至 instance**；規則：
  - **同一 branch 內後續 step** 立即可見本 branch 前面的 assign 寫入
  - **其他 branch 內 step** 不保證可見；可能看到初始值或其他 branch 的部分寫入（並發 race）
  - **parallel 結束後的 step** 可見所有 branch 的最終寫入（last-write-wins by assign 完成時間）
  - 若需確定性跨 branch 可見性，請勿於 parallel 內寫入 `vars`（改以 branch 的 step output 透過 `steps.<id>.output` 讀取，但需配合 `steps.*` 僅可見「本 branch」的限制，故跨 branch 共享仍建議於 parallel 前後處理）
...
> **注意 — emit 與 trigger 的預設 scope 不對稱**：
>
> - `emit.scope` 預設 `workflow`（隔離為安全預設，避免事件意外跨 instance 洩漏）
> - `trigger.scope` 預設 `project`（見 `02-workflow.md` 的 event trigger）
>
> 若 emit 省略 scope（= `workflow`）發出事件，**無法被任何 trigger 匹配**（trigger 不可訂閱 `scope: workflow`）。若欲跨 instance 通訊，emit **MUST** 顯式宣告 `scope: project` 或 `global`。同 instance 內 emit → wait 通訊（self-emit）則 `scope: workflow` 足夠（short-circuit 路徑，不離開 instance）。
...
**scope:project 匹配以 `project.path` 為準**（非 `project.name`；避免不同 hierarchy 下同名 project 誤匹配，見 `runtime-spec/07-event-bus.md` 的 project 匹配規則）：

**根目錄 definition（`project.path == ""`）的 scope 語意**：

- `scope: workflow`：語意不變（僅當前 instance 內流通）
- `scope: project`：由於根目錄無 project，路由時等價於「僅匹配同屬根目錄 definition（`project.path == ""`）的 trigger / wait」；**不**自動升級為 `global`，亦不會匹配任何具名 project 內的訂閱者
- `scope: global`：語意不變（跨所有 project 與根目錄 definition）
- 根目錄 definition 的 event trigger / wait 亦套用同規則：`scope: project` 的 trigger 僅匹配根目錄（`project.path == ""`）emit 出的 `scope: project` 事件

此設計讓根目錄 definition 構成一個隱含的「default project」隔離域，避免 prototyping 階段的事件意外洩漏至 production project，亦避免被 production project 的 `scope: project` 事件誤觸發。
...
- **保留前綴**（engine 內部使用，`emit` 不得宣告）：
  ...
  - `tool.*`：tool backend 事件（如 `tool.callback`）
  - `step.*`、`instance.*`、`wait.*`、`event.*`、`lifecycle.*`、`trigger.*`、`workflow.*`、`function.*`：引擎內部流程事件
- 違反格式或使用保留前綴 → 載入失敗，`registry.invalid_event_name`
- `wait.signals[].event` / event trigger 的 `event` 欄位亦套同一規則；所有保留前綴（`engine.*` / `internal.*` / `bus.*` / `tool.*` / `step.*` / `instance.*` / `wait.*` / `event.*` / `lifecycle.*` / `trigger.*` / `workflow.*` / `function.*`）皆不可被使用者 wait / trigger 訂閱
...
#### Secret 禁用於 emit.data（安全規則）

`emit.data` 的 CEL 表達式 **MUST NOT** 引用 `secret.*` namespace：

- **載入期靜態檢查**：engine 於 workflow / function definition 載入時掃描 `emit.data` 所有 CEL fragment 的 AST；若任一節點引用 `secret.<KEY>` 或 `secret["<KEY>"]` → 載入失敗，`error.type: "registry.secret_in_emit_data"`，`error.details.path` 為 dotted 路徑（如 `steps[3].data.api_key`）
- **理由**：emit 事件會寫入 event bus / delayed_events / dead_letter / 訂閱者 instance，無法保證跨 instance 傳遞時的 redaction 一致性；任何 `secret.*` 於 emit.data 皆視為潛在外洩
- **替代做法**：若需在事件中帶入「來自 secret 的派生值」（如雜湊、簽章），請以 `type: task` 呼叫 tool 產生（tool 內部可安全存取 secret），再將 tool output 放入 `emit.data`；tool 的 output 不受 secret 禁用規則限制（tool 作者自負 redaction 責任）
- **execution log redaction 仍作為最後防線**：即便靜態檢查漏網（如動態 lambda / 字串拼接），log 層的 SecretAccessor 追蹤仍會將明文子串遮蔽為 `"***"`（見 `runtime-spec/04-expression-evaluation.md` 的「錯誤訊息中的 secret 防洩漏」與 `08-persistence.md` redaction 規則）；但已投遞至訂閱者的事件**無法事後撤回**
- 同規則適用於 `emit.event`（事件類型名）與 `emit.delay`（duration）的 CEL，雖然這兩個欄位通常不含敏感資料，但為規則簡化一致禁用
...
| `match` | CEL expression | MAY | 事件過濾條件；回傳 boolean |

**match 求值規則**：

- 每次事件到達時求值（不是建立 subscription 時）
- 求值結果為 `true` → 該事件觸發本 wait 喚醒
- 求值結果為 `false` → 該事件不匹配；subscription 保持等待下一個事件
- 求值**異常**（identifier_not_found / type_error / ...）→ **視為不匹配**（等同 false），engine 記錄 `subscription.match_eval_failed` observability 事件但 **不** 令 wait step FAILED；subscription 保持等待
  - 此設計讓 `match` 可以對「可能缺欄位」的事件類型做防禦；例：`match: ${ has(event.data.order_id) && event.data.order_id == input.order_id }`（不用 has 檢查時，event.data 無 order_id 會靜默跳過該事件）
- 非 boolean 結果（如 string / int）→ 同上視為不匹配（不做 truthy 自動轉換，避免誤觸發）
...
- `signals[].step` 的值 MUST 指向**同一 workflow definition** 內定義的 step ID，且該目標 step MUST 為 `async: true` 的 `foreach` / `parallel`；其他類型（`task` / `wait` / `emit` / ...）或 `async: false` 的 step 不可作為訊號目標
- 目標 step 於 AST 中需位於本 wait step **語法上可達的作用域**：可指向同層或外層作用域中、語法上**先於** wait step 的 async step；不可指向兄弟 branch、其他迭代、或內層 step
- 違反上述任一項 → 載入失敗，`registry.invalid_signal_target`；`error.details.step` 為違反的 step id
- 重載（hot reload）後若原目標 step 在新版 definition 中消失：已存在的 wait subscription 於 engine 偵測時以 `error.type == "invalid_signal_target"` 令 wait step FAILED（見 `runtime-spec/07-event-bus.md`）
...
| 欄位 | 說明 |
|------|------|
| `steps.<id>.output.signal` | 匹配到的訊號類型：`"event"` / `"step"` / `"duration"`（duration 模式下恆為 `"duration"`） |
| `steps.<id>.output.event` | 事件訊號匹配時：事件類型（如 `payment.confirmed`）；step / duration 訊號時為 `null` |
| `steps.<id>.output.step` | step 訊號匹配時：step ID；事件 / duration 訊號時為 `null` |
| `steps.<id>.output.data` | 事件訊號匹配時：`event.data`；step 訊號匹配時：該 step 的 output；duration 模式無此欄位 |
| `steps.<id>.output.duration_ms` | duration 模式下：實際 sleep 毫秒數；signals 模式下無此欄位 |
...
- Output：`{ signal: "duration", duration_ms: <實際 sleep 毫秒> }`；正常結束時 `duration_ms` 恰為求值結果；被 cancel 中斷不產 output（step 終態為 FAILED，`error.type == "cancelled"`；step 層無獨立 CANCELLED 狀態，見 `runtime-spec/04-expression-evaluation.md` StepRef 可見狀態）
...
`saga.steps` MUST 為**非空陣列**；`steps: []` → 載入失敗，`registry.invalid_workflow_definition`、`details.reason: "empty_saga_steps"`。空 saga 無意義（無 step 可補償、無業務邏輯），不提供 fall-through 語意。

**允許的 step type**（載入驗證白名單）：

| 允許 | 說明 |
|------|------|
| `task` | 主要業務單元；SUCCEEDED 後 compensate 進入補償 log |
| `assign` | 寫入 vars（無 compensate 語意） |
| `if` / `switch` | 分支控制；分支內 task 的 compensate 依執行路徑累計（見「saga 內 `if`/`switch` 補償規則」） |
| `foreach` / `parallel` | 容器；內部 task 的 compensate 依時戳降序（見補償規則） |
| `emit` | 允許；但 **不可撤銷**（見下「Saga 內的 `type: emit` 語意」） |

**禁止的 step type**（載入期拒絕，`registry.invalid_saga_step_type`）：

| 禁止 | 理由 |
|------|------|
| `wait` | wait 的恢復語意與 saga 補償狀態機衝突（如補償期間仍在等待事件，無明確終結點） |
| `callback` | callback 僅 Function 內使用；saga 可巢狀於 function，但 saga.steps 直接含 callback 會使補償序列不確定（handler 在 caller 執行） |
| `return` | return 會提前終結 function / workflow，繞過 saga 的 SUCCEEDED 判定；若需提前終結請將 saga 包在 if 內並以 if-then-return 決定 |
| `fail` | 同理；`fail` 應置於 saga 外或以 `type: task` 呼叫 builtin fail tool 替代 |
| `saga`（巢狀） | 允許（見「巢狀 saga」規則），非禁止 |

違反白名單 → 載入失敗，`error.type: "registry.invalid_saga_step_type"`、`details.step_type: <violating_type>`、`details.step_path: <path_in_saga>`。
...
- 若 saga 內含 `if` / `switch`：
  - **容器 step 本身無 compensate**：`if` / `switch` 作為控制流不產生副作用，不進入 compensation log；僅其被選中分支內部執行的 task step 的 compensate 進入 log
  - **未選中分支** 的 step 不執行、不計入 compensation log（無 timestamp）；`else: []` / 未選中 case / `condition: false` 的分支整體跳過
  - 被選中分支內的 step 依其 SUCCEEDED 時間戳加入 compensation log；補償時與 saga 其他 step 統一降序處理
  - **巢狀 if / switch**：內層分支內 step 的補償時間戳即該 step 本身的 SUCCEEDED 時戳；容器結構不參與時戳計算
