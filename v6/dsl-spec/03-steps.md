...
- `wait.signals[].event` / event trigger 的 `event` 欄位亦套同一規則；**例外**：`wait.signals[].event: tool.stream` 明示允許（用於訂閱當前 instance 內 tool 的 stream 訊息），此為唯一可被使用者 wait 訂閱的保留前綴事件。其他保留前綴（`engine.*` / `internal.*` / `bus.*` / `tool.callback` / `step.*` / `instance.*` / `wait.*` / `event.*` / `lifecycle.*` / `trigger.*` / `workflow.*` / `function.*`）仍禁用
