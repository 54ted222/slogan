...
| `toolset.unresolved_reference` | 引用不存在的 sub-toolset / action |
| `toolset.circular_include` | 循環 include |
...
| `agent.tools_conflict` | step 同時設 `tools` 與 `tools_override` |
| `max_iterations_exceeded` | agent.config.max_iterations 達上限 |
| `max_tool_calls_exceeded` | agent.config.max_tool_calls 達上限 |
...
所在控制結構（if / switch / foreach / parallel）的失敗策略
  ...
  ▼
父 step（saga / agent step / function 呼叫端 task）
  │ saga → 進入補償
...
`config.catch` 內只能用 `error.*` / `input` / `vars` / `steps` 等已凍結的 namespace；不可再執行 `wait` / `agent` 等需要長時間 I/O 的 step（實作可拒絕；建議僅允許 emit/fail/return/assign）。
