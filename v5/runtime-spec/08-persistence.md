...
|------|------|------|
| `id` | uuid | 主鍵 |
| `parent_id` | uuid \| null | 父 instance（agent session / function instance 才有） |
| `kind` | enum | `workflow` / `agent_session` / `function_instance` |
...
每個 project 可以在 project.yaml 設定覆寫保存期（v5 目前未指定 yaml schema；保留為實作擴充）。
