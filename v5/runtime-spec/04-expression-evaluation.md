...
```
{
  input:     map | null,         # workflow / agent / function instance 的初始 input
  steps:     map<step_id, StepRef>,
  prev:      StepRef | null,     # 語法上的前一步
  vars:      map,                # 所在 instance 的 vars
  loop:      LoopFrame | null,
  session:   SessionCtx | null,  # 僅 agent 內
  ...
  config:    map | null,          # agent 自訂 steps 內可用
  templates: map<string, string>, # Resources string_templates
}
```
