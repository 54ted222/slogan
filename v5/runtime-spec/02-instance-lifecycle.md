...
本文件定義 workflow / agent / function instance 的狀態機、轉換規則與持久化時機。
...
| **Agent Session** | `type: agent` step / `type: task` 呼叫 agent | agent `return` step、自然 fall-through、上限 / 錯誤 |
| **Function Instance** | `type: task` 呼叫 function | `return` step、unhandled error、外層 timeout |

Agent session 與 function instance 由父 instance「擁有」；父 instance 若被取消，子 instance 同步取消。
...
- 終結前：
  1. 求值 `output_schema`（若 instance 為 workflow / function / agent；session 無 schema 則跳過）
...
### RUNNING → FAILED
- 觸發點：未被任何 `catch` / saga 捕捉的錯誤、`type: fail` step、`config.timeout`、`max_iterations_exceeded` 等。
...
```
Workflow Instance
 ├─ Step (type: agent) → Agent Session
 │    └─ Step (type: task action: agent.exec_tool_calls)
 │         └─ Tool 子呼叫 (各自一個 Tool execution，但不算 instance)
 │
 ├─ Step (type: task action: <function>) → Function Instance
 │    └─ Step (type: callback)  ← 暫停，等待 caller handler
 │
 └─ Step (type: task action: <agent>) → Agent Session（不可覆寫）
```
...
| Sub-instance（agent session / function instance） | 隨父 instance 一同清除 |
