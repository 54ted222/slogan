...
最終生效的 model config 由多層合併而成，優先級由低到高（後者覆寫前者）：

1. Resources 中 alias 定義的 `config`
2. Agent Definition 的 `model.config`（若以 alias + 覆寫形式指定）
3. Agent Step 的 `model.config`（若 step 提供 `model:`）

合併為 deep merge：map 逐欄覆寫，scalar / list 整體替換。例：`alias.config.max_tokens = 4096`、`definition.config.max_tokens = 8192` → 生效值 `8192`。
...
| `tools`    | 見下節「tools 合併順序」                             |

### tools 合併順序

```
1. Agent Definition 的 tools
2. Step 層級的 tools（append；與 1 做聯集，同 action name 以 step 為準）
3. Step 層級的 tools_override（若有，完全取代 1+2，不做聯集）
4. Step 層級的 tools_exclude（從最終結果移除指定 action name）
```

> `tools` 與 `tools_override` 互斥：同時出現時引擎 MUST 在載入時回報 validation error `agent.tools_conflict`。`tools_exclude` 可與任一搭配使用。
